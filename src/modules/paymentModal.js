...
const paymentFormTxStruct = [["amountBTC", "amount"], "address", "bip21Link"];

const paymentFormTextAreas = ["memo"];

const paymentFormLinks = ["bip21Link"];

const paymentFormQRCodeEnabled = ["bip21Link"];

const paymentFormCopyEnabled = ["amountBTC", "amount", "address"];

function genInputOrTextArea(dataKey, dataVal) {
  if (paymentFormTextAreas.indexOf(dataKey) > -1) {
    return $(
      `<textarea class="form-control form-control-lg" readonly>${dataVal}</textarea>`
    );
  } else {
    const $input = $(
      `<input type="text" class="form-control form-control-lg" value="${dataVal}" readonly>`
    );
...
function paymentDataToModalContentItem(data, item) {
  ...
  if (typeof item === typeof []) {
    ...
    if (paymentFormLinks.indexOf(item) > -1) {
      ...
      const $itemBtn = $(
        `<div><a href="${data[item]}" class="btn btn-lg btn-primary">${paymentFormLabels[item]}</a><div>`
      );
      $item.append($itemBtn);
    } else {
      $item = $(`
...
export function paymentDataToModalContent(data) {
  ...
  data.outputs.forEach((output) => {
    ...
    output.bip21Link = encodeBip21Link(
      data.currency === "BCH",
      output.address,
      output.amountBTC,
      "Bitpay",
    ...
    paymentFormTxStruct.forEach((item) => {
      $content.append.apply(
        $content,
        paymentDataToModalContentItem(output, item)
      );
    });
  });
  $content.append(
    '<div class="alert alert-info mt-3" role="alert">' +
      '💬 Paid the invoice? Please <a href="https://github.com/alexk111/DeBitpay/issues/23" target="_blank" rel="noopener noreferrer">post a comment</a> to let others know that it still works.' +
      "</div>"
  );
  return $content;
}
